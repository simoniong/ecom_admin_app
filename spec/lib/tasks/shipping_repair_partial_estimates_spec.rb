...
RSpec.describe "shipping:repair_partial_estimates", type: :task do
  before(:all) do
    Rake.application.rake_require("tasks/shipping", [ Rails.root.join("lib").to_s ])
    Rake::Task.define_task(:environment)
  end

  let(:task) { Rake::Task["shipping:repair_partial_estimates"] }
  ...
  it "reports the repair and writes nothing without APPLY" do
    order = build_order(weights_grams: [ 800, 400 ], estimated_shipping_cost: usd_for(0.8))

    expect { task.invoke }.to output(
      match(/DRY RUN/)
        .and(match(/scanned=1 repairable=1 unexplained=0/))
        .and(match(/#{Regexp.escape(order.name)}\s+#{Regexp.escape(usd_for(0.8).to_s)}\s+->\s+#{Regexp.escape(usd_for(1.2).to_s)}/))
        .and(match(/proven by 1\/2 items/))
    ).to_stdout

    expect(order.reload.estimated_shipping_cost).to eq(usd_for(0.8))
  end

  it "writes the repair with APPLY=1" do
    order = build_order(weights_grams: [ 800, 400 ], estimated_shipping_cost: usd_for(0.8))
    ENV["APPLY"] = "1"
  ...
  it "lists an unprovable order under Unexplained and leaves it untouched" do
    ...
    expect { task.invoke }.to output(
      match(/Unexplained/)
        .and(match(/#{Regexp.escape(order.name)}\s+frozen=99\.99/))
    ).to_stdout
